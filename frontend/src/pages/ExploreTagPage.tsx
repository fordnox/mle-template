...
export default function ExploreTagPage() {
  ...
  const pageTitle = `Schemas tagged "${decodedTag}" | ${config.VITE_APP_TITLE}`
  const pageDescription = `Browse database schemas tagged with "${decodedTag}". Find ERD diagrams and DBML schemas.`
  const pageUrl = `${config.VITE_APP_URL}/explore/tag/${encodeURIComponent(decodedTag)}`
    ...
    <>
      ...
      <main className="flex-1 container mx-auto px-6 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
          ...
          <span className="text-foreground font-medium">Tag: {decodedTag}</span>
        </nav>

        {/* Page Header */}
        <section className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-foreground mb-2">
            Schemas tagged "{decodedTag}"
          </h1>
